...
- [x] **Step 1: `Teleframe` facade class.** Constructor `(ContainerInterface $container)` or explicit composed services; exposes:
...
- [x] **Step 1: Full gate.** `composer verify` green; `TELEFRAME_PG=1 vendor/bin/phpunit tests/Pg` green (where env-gated on this box); `php bin/standalone-smoke.php` exit 0 (now including the facade); `php bin/regenerate` idempotent (handlers add no generated surface, cheap re-check). The definitive phase assertion: `FakeRunningModeTest` proves a fake uprate flows the REAL pipeline (registry + onion + echo elimination + DI hydration) — no handler-substrate stub.
- [x] **Step 2: roadmap + spec ticks.** Tick Phase 3 "Write plan" + "Gate: fake uprate flows the real pipeline in tests; facade one-class DX documented; gates green" checkboxes; append `Phase 3 (Handler Substrate) COMPLETE 2026-09-0X` to the unification spec status line with the gate evidence.
- [x] **Step 3: commit + push** teleframe `main`; update Nowledge Mem (memory `97c4c8af-f18a-4b6f-89e7-551f8ed2c1aa`) with Phase 3 completion.
