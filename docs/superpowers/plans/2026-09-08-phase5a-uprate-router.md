...
- [ ] **Task 1: sscanf arg receipt.** `HandlerMatcher` match result carries
...
- [ ] **Task 2: uprate identity + replay dedup.** `Update::updateId()` =
...
- [ ] **Task 3: `ValidatedUpdate` (FormRequest analog).** Opt-in DI wrapper over
...
- [ ] **Task 4: docs.** `docs/handlers.md`: args receipt, validation wrapper,
...
- [ ] **Task 5: gate.** `composer verify` exit 0 (phpstan clean), smoke exit 0,
      ...
      status line updated; commit + push `main`.
