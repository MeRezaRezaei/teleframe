...
- Source: tlType=`ChannelParticipant`, 6 ctors (`channelParticipant`, `channelParticipantSelf`, `channelParticipantCreator`, `channelParticipantAdmin`, `channelParticipantBanned`, `channelParticipantLeft`), PK=`[channel_participant_id]`, constructor discriminator: yes.
- **Key derivation note:** the synthetic PK `channel_participant_id` comes from the resolver's `deriveKeyColumns` case 3 — base[0] is `user_id` (neither `id` nor `peer`), so a synthetic key is derived from the table name singular. It is a documented derivable artifact, not a catalog value.
- Base columns: `channel_participant_id BIGINT PK`; `user_id BIGINT`; `date INTEGER`. No bools.
