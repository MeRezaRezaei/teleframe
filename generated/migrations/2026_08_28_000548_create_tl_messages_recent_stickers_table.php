...
return new class extends Migration
{
    ...
    {
        Schema::create('tl_messages_recent_stickers', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->bigInteger('constructor_id'); // crc32, may exceed signed i32
            $table->string('constructor_name', 96);
            $table->bigInteger('account_id'); // tenant (roadmap: account_id on every anchor)
            $table->timestamps();
            $table->index('constructor_id', 'ix_41204a593bcfbe81c8cf4b39');
            $table->index('account_id', 'ix_e601634b0034693bd439aa92');
        });
        Schema::create('tl_messages_recent_stickers_recent_stickers', function (Blueprint $table) {
            $table->foreignUuid('id')->primary()->constrained('tl_messages_recent_stickers')->cascadeOnDelete();
        ...
        Schema::create('tl_messages_recent_stickers_recent_stickers__packs', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->foreignUuid('parent_id')->constrained('tl_messages_recent_stickers_recent_stickers')->cascadeOnDelete();
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('tl_messages_recent_stickers');
    }
};
