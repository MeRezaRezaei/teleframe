...
return new class extends Migration
{
    ...
    {
        Schema::create('tl_input_channel', function (Blueprint $table) {
            $table->uuid('id')->primary();
            $table->bigInteger('constructor_id'); // crc32, may exceed signed i32
            $table->string('constructor_name', 96);
            $table->bigInteger('account_id'); // tenant (roadmap: account_id on every anchor)
            $table->timestamps();
            $table->index('constructor_id', 'ix_8bfbb7f6ad85e138d23043c5');
            $table->index('account_id', 'ix_703b82a677e355e9b4645ad4');
        });
        Schema::create('tl_input_channel_input_channel', function (Blueprint $table) {
            $table->foreignUuid('id')->primary()->constrained('tl_input_channel')->cascadeOnDelete();
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('tl_input_channel');
    }
};
