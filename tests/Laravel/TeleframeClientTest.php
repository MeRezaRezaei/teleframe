...
namespace MeRezaRezaei\Teleframe\Laravel\Tests;
...
class TeleframeClientTest extends TestCase
{
    ...
    {
        $client = new TeleframeClient(defaultApiId: 111, defaultApiHash: 'hash');
    ...
    {
        $client = new TeleframeClient(defaultBotToken: 'default:bot_token');
    ...
    public function testBotClientWithHttpFake(): void
    {
        ...
        $bot = new \MeRezaRezaei\Teleframe\Bot\Services\BotClient('12345', http: $httpFactory);
        $me = $bot->call('getMe');
    ...
    {
        $keyboard = \MeRezaRezaei\Teleframe\Core\Types\InlineKeyboard::make()
            ->row([
                \MeRezaRezaei\Teleframe\Core\Types\InlineKeyboard::urlButton('Website', 'https://example.com'),
                \MeRezaRezaei\Teleframe\Core\Types\InlineKeyboard::callbackButton('Click Me', 'btn_clicked'),
            ])
            ->row([
                \MeRezaRezaei\Teleframe\Core\Types\InlineKeyboard::webAppButton('Open App', 'https://app.example.com'),
            ]);
        ...
        $photo = \MeRezaRezaei\Teleframe\Core\Types\InputMedia::photo('https://example.com/photo.jpg', 'Cool Photo');
        ...
        $video = \MeRezaRezaei\Teleframe\Core\Types\InputMedia::video('https://example.com/video.mp4', 'Cool Video');
        $this->assertEquals('video', $video['type']);

        $replyKeyboard = \MeRezaRezaei\Teleframe\Core\Types\ReplyKeyboard::make()
            ->row([\MeRezaRezaei\Teleframe\Core\Types\ReplyKeyboard::requestContact('Send My Phone')])
        ...
        $remove = \MeRezaRezaei\Teleframe\Core\Types\ReplyKeyboard::remove(true);
    ...
    public function testBotMtprotoScope(): void
    {
        ...
        $this->assertInstanceOf(\MeRezaRezaei\Teleframe\Bot\Services\BotAccountScope::class, $botMtproto);
    ...
    public function testTerminalQrRendering(): void
    {
        ...
        $qrString = \MeRezaRezaei\Teleframe\Core\Support\TerminalQr::render($url);
        ...
        $fallback = \MeRezaRezaei\Teleframe\Core\Support\TerminalQr::renderOrUrl($url);
        $this->assertStringContainsString($url, $fallback);
    }

    public function testFloodWaitExceptionExposesSeconds(): void
    {
        $e = new \MeRezaRezaei\Teleframe\Core\Exceptions\Rpc\FloodWaitException(17, 'FLOOD_WAIT_17', 420);
    ...
    public function testAesIgeRejectsNonBlockMultiple(): void
    {
        ...
        \MeRezaRezaei\Teleframe\Core\MTProto\Crypto\AesIge::encrypt('short', random_bytes(32), random_bytes(32));
    }

    public function testUserSpecificMtprotoMethods(): void
    {
        $client = new TeleframeClient(defaultApiId: 7777, defaultApiHash: 'hash7777');
        $user = $client->user(12345, random_bytes(256));
        ...
        // Import contacts
        $contact = \MeRezaRezaei\Teleframe\Core\Types\InputContact::phone('+1234567890', 'John', 'Doe');
    ...
    public function testTelegramUpdateReceivedEvent(): void
    {
        ...
        $event = new \MeRezaRezaei\Teleframe\Laravel\Events\TelegramUpdateReceived($rawUpdate, '123456:BOT');
    ...
    {
        $controller = new \MeRezaRezaei\Teleframe\Laravel\Http\Controllers\TelegramWebhookController();
    ...
    public function testUpdatePollerServiceWithCustomSinkAndFilter(): void
    {
        ...
        $customSink = new class($capturedUpdates) implements \MeRezaRezaei\Teleframe\Core\Contracts\UpdateSinkInterface {
        ...
        $poller = new \MeRezaRezaei\Teleframe\Laravel\Services\UpdatePollerService($customSink);
    ...
    public function testTeleframeAuthServiceMethods(): void
    {
        $authService = new \MeRezaRezaei\Teleframe\Laravel\Services\TeleframeAuthService(live: false);
        ...
        $this->assertInstanceOf(\MeRezaRezaei\Teleframe\Core\MTProto\SessionData::class, $phoneRes['session']);
    ...
    public function testDispatchRoutesBotHttpRequestThroughBotClientHttp(): void
    {
        ...
        $client = new TeleframeClient(defaultApiId: 1, defaultApiHash: 'h', defaultBotToken: '12345', http: $http);
        $res = $client->dispatch(\MeRezaRezaei\Teleframe\Core\Methods\Methods::bots()->getMe()->toRequest());
    ...
    {
        $client = new TeleframeClient(defaultApiId: 1, defaultApiHash: 'h', defaultUserSession: 'offline-unit-auth-key');

        $res = $client->dispatch(
            \MeRezaRezaei\Teleframe\Core\Methods\Methods::messages()->sendMessage()
    ...
    {
        $client = new TeleframeClient(defaultApiId: 1, defaultApiHash: 'h', defaultBotToken: 't');
