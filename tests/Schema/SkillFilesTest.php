...
namespace MeRezaRezaei\Teleframe\Schema\Tests;

use PHPUnit\Framework\TestCase;

class SkillFilesTest extends TestCase
{
    protected function getSkillDir() { return dirname(__DIR__, 2) . '/src/Schema/skills/telegram-methods'; }

    public function testSkillFileRenderedForCuratedMethod(): void
    {
        $path = dirname(__DIR__, 2) . '/src/Schema/skills/telegram-methods/messages.sendMessage.md';
    ...
    {
        $this->assertFileExists($this->getSkillDir() . '/messages.sendMessage.md');
        $md = (string) file_get_contents($this->getSkillDir() . '/messages.sendMessage.md');
        $lines = explode("\n", $md);
    ...
    {
        $path = $this->getSkillDir() . '/sendMessage.md';
        $this->assertFileExists($path);
        $md = (string) file_get_contents($path);
    ...
    public function testMultiPlaceholderTemplateErrorRendersRealDescription(): void
    {
        ...
        $md = (string) file_get_contents($this->getSkillDir() . '/messages.forwardMessages.md');
