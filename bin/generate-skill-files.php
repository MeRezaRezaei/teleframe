...
$renderUsage = static function (TelegramMethod $m) use ($groupOf, $accessorOf, $camel, $placeholderFor): array {
    ...
    $lines[] = '    ->toRequest();';
    $lines[] = '';
    // dispatch() is an INSTANCE method on TeleframeClient (it resolves the
    // transport per method: 'mtproto' -> user(), 'bot-http' -> bot()), so the
    // example must construct/resolve a client first — never a static call.
    $lines[] = '$client = app(\MeRezaRezaei\Teleframe\Laravel\Services\TeleframeClient::class);   // or: new TeleframeClient(defaultApiId: …, defaultApiHash: …)';
