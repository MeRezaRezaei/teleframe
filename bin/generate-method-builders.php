...
use MeRezaRezaei\Teleframe\Core\Schema\TelegramMethod;

const GENERATED_NS = 'MeRezaRezaei\\Teleframe\\Core\\Methods\\Generated';
const GENERATED_NS_BOT = 'MeRezaRezaei\\Teleframe\\Bot\\Methods\\Generated';
const HEADER_DATE = '2026-08-28';

$checkOnly = in_array('--check', $argv, true);
$drift = false;

$configPath = __DIR__ . '/../src/Schema/config/curated-methods.json';
$outDir = dirname(__DIR__) . '/src/Core/Methods/Generated';
 ...
 */
$renderGroup = static function (string $groupClass, string $groupLabel, array $entries, string $ns) use ($renderBuilder): string {
    $lines = [];
    $lines[] = '/**';
    $lines[] = ' * ' . $groupLabel . '.';
    $lines[] = ' *';
...
foreach ($groups as $groupKey => $group) {
    $groupNs = $group['api'] === 'bot-http' ? GENERATED_NS_BOT : GENERATED_NS;
    foreach ($group['entries'] as $entry) {
        $fqcn = $groupNs . '\\' . $entry['builder'];
    ...
    if (isset($declared[$groupNs . '\\' . $group['class']])) {
...
foreach ($groups as $groupKey => $group) {
    $targetDir = $group['api'] === 'bot-http'
        ? dirname(__DIR__) . '/src/Bot/Methods/Generated'
        : $outDir;

    $label = $group['api'] === 'bot-http'
        ? 'Bot API (bot-http) curated method builders'
        : 'mtproto ' . $groupKey . '.* curated method builders';
    $file = $renderGroup($group['class'], $label, $group['entries'], $group['api'] === 'bot-http' ? GENERATED_NS_BOT : GENERATED_NS);
