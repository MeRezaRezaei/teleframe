...
{
    public function __construct(string $rpcErrorMessage = 'SESSION_PASSWORD_NEEDED', int $rpcErrorCode = 401, ?string $method = null)
    {
        parent::__construct($rpcErrorMessage, $rpcErrorCode, '2FA is enabled on this account. → Fetch account.getPassword and complete the SRP flow (auth.checkPassword).', $method);
    }
}
